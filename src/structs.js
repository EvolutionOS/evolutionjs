const {Signature, PublicKey} = require('evojs-ecc')
...
/** Configures Fcbuffer for EVO specific structs and types. */
module.exports = (config = {}, extendedSchema) => {
  ...
  const customTypes = Object.assign({}, evoTypes, config.customTypes)
...
function precisionCache(assetCache, value) {
  ...
  const contract = symbolInfo.contract || 'evo.token'
...
const abiOverride = structLookup => ({
  ...
  'setabi.abi.appendByteBuffer': ({fields, object, b}) => {
    const ser = structLookup('abi_def', 'evo')
