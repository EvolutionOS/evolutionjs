...
const {processArgs} = require('evojs-api')
...
function writeApiGen(Network, network, structs, config, schemaDef) {
  ...
  /**
    ...
    @example evo.contract('mycontract', [options], [callback])
    @example evo.contract('mycontract').then(mycontract => mycontract.action(...))
  */
  merge.contract = (...args) => {
...
function WriteApi(Network, network, config, Transaction) {
  ...
  */
  const genTransaction = (structs, merge) => async function(...args) {
    ...
    if(isContractArray) {
      ...
      const abiPromises = []
      // Evo contract operations are cached (efficient and offline transactions)
      const cachedCode = new Set(['evo', 'evo.token'])
  ...
  function genMethod(type, definition, transactionArg, account = 'evo.token', name = type) {
  ...
  */
  function trMessageCollector(trCallback, options = {}, merges) {
    ...
    // merges can be an object of functions (as in the main evo contract)
    ...
    for(const key in merges) {
      ...
      if(typeof value === 'function') {
        // Native operations (evo contract for example)
...
function usage (type, definition, Network, account, config) {
  ...
  let struct
  if(account === 'evo' || account === 'evo.token') {
